// Blynk Configuration
#define BLYNK_TEMPLATE_ID "TMPL6DgKqxkg8"
#define BLYNK_TEMPLATE_NAME "Car Air Sentinel"
#define BLYNK_AUTH_TOKEN "uilAYhKPf8XLyWf5mEBqQfwBUiAqKF06"
...
void sendToBlynk(float CO, float CO2, String status, int relayStatus) {
  Blynk.virtualWrite(V1, CO2);          // Send CO2 data to Blynk
  Blynk.virtualWrite(V0, CO);           // Send CO data to Blynk
  Blynk.virtualWrite(V3, status);       // Send status to Blynk
  Blynk.virtualWrite(V2, relayStatus);  // Send relay status to Blynk
}
...
void setup() {
  ...
  timer.setInterval(1000L, []() {
    ...
    if ((CO < 50) && (CO2 < 1000)) {
      ...
      digitalWrite(GREEN_PIN, HIGH); // LED Hijau hidup
      ...
      if (WiFi.status() != WL_CONNECTED) {
        ...
        if (currentMillis - previousMillis >= 1000) {
          ...
          digitalWrite(GREEN_PIN, LOW); // LED Hijau mati
          delay(1000);
